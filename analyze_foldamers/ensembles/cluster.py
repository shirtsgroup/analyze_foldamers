...
def get_representative_structures(
    ...
    for file in file_list:
        distances, traj_all = get_rmsd_matrix(
            file, cgmodel, frame_start, frame_stride, frame_end
        )
...
def get_cluster_medoid_positions_KMedoids(
    ...
    if plot_rmsd_hist:
        ...
        figure = plt.figure()
        n_out, bin_edges_out, patch = plt.hist(distances_row, bins=1000, density=True)
        plt.xlabel("rmsd")
    ...
    write_medoids_to_file(
        cgmodel, medoid_xyz, output_dir, output_format, top_from_pdb=top_from_pdb
    )
...
def get_cluster_medoid_positions_DBSCAN(
    ...
    dbscan = DBSCAN(min_samples=min_samples, eps=eps, metric="precomputed").fit(
        distances
    )
    ...
    for k in range(n_clusters):
        cluster_rmsd[k] = np.sqrt(
            ((distances_k[k][intra_cluster_medoid_index[k]] ** 2).sum())
    ...
    try:
        ...
        if plot_silhouette:
            # Plot silhouette analysis
            plotfile = (
                f"{output_dir}/silhouette_dbscan_min_sample_{min_samples}_eps_{eps}.pdf"
            )
...
def get_cluster_medoid_positions_OPTICS(
    ...
    optic = OPTICS(
        min_samples=min_samples, xi=xi, cluster_method="xi", metric="precomputed"
    ).fit(distances)
    # The produces a cluster labels from 0 to n_clusters-1, and assigns -1 to noise points
    ...
    try:
        ...
        if plot_silhouette:
            # Plot silhouette analysis
            plotfile = (
                f"{output_dir}/silhouette_optics_min_sample_{min_samples}_xi_{xi}.pdf"
            )
        ...
        print(
            "There are either no clusters, or no noise points identified. Try adjusting OPTICS min_samples, xi parameters"
        )
        silhouette_avg = None
...
def filter_distances(
    distances, filter_ratio=0.05, return_original_indices=False, original_indices=None
):
    """
    ...
    for k in range(n_clusters):
        for j in range(k + 1, n_clusters):
            ...
            if nrow > 1 and ncol > 1:
                ...
                axs2[row, col].set_xlabel(f"Distance to centroid {k}", {"fontsize": 8})
                axs2[row, col].set_ylabel(f"Distance to centroid {j}", {"fontsize": 8})
            ...
            if ncol > 1 and nrow == 1:
                ...
                axs2[col].set_xlabel(f"Distance to centroid {k}", {"fontsize": 8})
                axs2[col].set_ylabel(f"Distance to centroid {j}", {"fontsize": 8})
            ...
            if ncol == 1 and nrow == 1:
                ...
                axs2.set_xlabel(f"Distance to centroid {k}", {"fontsize": 8})
                axs2.set_ylabel(f"Distance to centroid {j}", {"fontsize": 8})
...
def get_rmsd_matrix(
    ...
    for i in range(len(file_list)):
        if file_list[0][-3:] == "dcd":
            rep_traj[i] = md.load(
                file_list[i], top=md.Topology.from_openmm(cgmodel.topology)
            )
        else:
    ...
    for i in range(len(file_list)):
        ...
        if return_original_indices:
            original_indices[i] = original_indices[i][
...
def write_medoids_to_file(
    cgmodel, medoid_positions, output_dir, output_format, top_from_pdb=None
):
    ...
    for k in range(n_clusters):
        ...
        if cgmodel is None:
            ...
            traj = md.Trajectory(
                xyz=positions.value_in_unit(unit.nanometer), topology=top
            )
...
def make_silhouette_plot(
    ...
    for k in range(n_clusters):
        ax1.text(
            xlim[0] + 0.75 * (xlim[1] - xlim[0]),
            (0.9 - (k / 25)) * (ylim[1] - ylim[0]),
