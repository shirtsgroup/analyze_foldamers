...
def cluster_torsions_KMedoids(
    ...
    torsion_val_array, traj_all = get_torsion_matrix(
        file_list, cgmodel, frame_start, frame_stride, frame_end, backbone_torsion_type
    )
    ...
    if filter:
        # Filter distances:
        torsion_distances, dense_indices, filter_ratio_actual = filter_distances(
            torsion_distances, filter_ratio=filter_ratio
        )
    ...
    kmedoids = KMedoids(n_clusters=n_clusters, metric="precomputed").fit(
        torsion_distances
    )
    ...
    if core_points_only:
        ...
        for k in range(n_clusters):
            intra_cluster_medoid_index.append(
                np.exp(-torsion_distances_k[k] / torsion_distances_k[k].std())
                .sum(axis=1)
                .argmax()
            )
