...
def assign_torsion_types(cgmodel, torsion_list):
    ...
    for i in range(len(torsion_list)):
        torsion_sub_arrays[str(torsion_types[i])][
            n_i[torsion_types[i] - 1], :
        ] = torsion_array[i, :]
...
def calc_bond_angle_distribution(
    ...
    for file in file_list:
        ...
        for i in range(i_angle_type):
            ...
            ang_val_array = (180 / np.pi) * np.reshape(
                ang_val_array, (nframes * n_i[i][0], 1)
            )
            ...
            angle_hist_data[file_key][
                f"{inv_ang_dict[str(i+1)]}_bin_centers"
            ] = angle_bin_centers
...
def calc_torsion_distribution(
    ...
    for file in file_list:
        ...
        for i in range(i_torsion_type):
            ...
            # Reshape arrays and convert to degrees:
            torsion_val_array = (180 / np.pi) * np.reshape(
                torsion_val_array, (nframes * n_i[i][0], 1)
            )
            ...
            torsion_hist_data[file_key][
                f"{inv_torsion_dict[str(i+1)]}_bin_centers"
            ] = torsion_bin_centers
...
def calc_2d_distribution(
    ...
    for file in file_list:
        ...
        if n_particle_x == 2:
            ...
            for i in range(i_bond_type):
                if (
                    inv_bond_dict[str(i + 1)] == xvar_name
                    or inv_bond_dict[str(i + 1)] == xvar_name_reverse
                ):
                    # Compute all bond length values in trajectory
                    # This returns an [nframes x n_bonds] array
                    xvar_val_array[file] = md.compute_distances(
                        traj, bond_sub_arrays[str(i + 1)]
                    )
            ...
            for i in range(i_angle_type):
                if (
                    inv_ang_dict[str(i + 1)] == xvar_name
                    or inv_ang_dict[str(i + 1)] == xvar_name_reverse
                ):
                    # Compute all angle values in trajectory
                    # This returns an [nframes x n_angles] array
                    xvar_val_array[file] = md.compute_angles(
                        traj, ang_sub_arrays[str(i + 1)]
                    )
            ...
            for i in range(i_torsion_type):
                if (
                    inv_torsion_dict[str(i + 1)] == xvar_name
                    or inv_torsion_dict[str(i + 1)] == xvar_name_reverse
                ):
                    # Compute all torsion values in trajectory
                    # This returns an [nframes x n_torsions] array
        ...
        if n_particle_y == 2:
            ...
            for i in range(i_bond_type):
                if (
                    inv_bond_dict[str(i + 1)] == yvar_name
                    or inv_bond_dict[str(i + 1)] == yvar_name_reverse
                ):
                    # Compute all bond length values in trajectory
                    # This returns an [nframes x n_bonds] array
                    yvar_val_array[file] = md.compute_distances(
                        traj, bond_sub_arrays[str(i + 1)]
                    )
            ...
            for i in range(i_angle_type):
                if (
                    inv_ang_dict[str(i + 1)] == yvar_name
                    or inv_ang_dict[str(i + 1)] == yvar_name_reverse
                ):
                    # Compute all angle values in trajectory
                    # This returns an [nframes x n_angles] array
                    yvar_val_array[file] = md.compute_angles(
                        traj, ang_sub_arrays[str(i + 1)]
                    )
            ...
            for i in range(i_torsion_type):
                if (
                    inv_torsion_dict[str(i + 1)] == yvar_name
                    or inv_torsion_dict[str(i + 1)] == yvar_name_reverse
                ):
                    # Compute all torsion values in trajectory
                    # This returns an [nframes x n_torsions] array
    ...
    for file in file_list:
        ...
        for iy in range(n_occ_y):
            xvar_val_array_combo[file][
                :, (iy * n_occ_x) : ((iy + 1) * n_occ_x)
            ] = xvar_val_array[file]
            for ix in range(n_occ_x):
                yvar_val_array_combo[file][:, ix + iy * n_occ_x] = yvar_val_array[file][
                    :, iy
                ]

        # Reshape arrays for histogramming:
        xvar_val_array_combo[file] = np.reshape(
            xvar_val_array_combo[file], (nframes * n_occ_x * n_occ_y, 1)
        )
        yvar_val_array_combo[file] = np.reshape(
            yvar_val_array_combo[file], (nframes * n_occ_x * n_occ_y, 1)
        )
    ...
    for file in file_list:
        ...
        for i in range(i_angle_type):
            if inv_ang_dict[str(i + 1)] == backbone_angle_type:
                # Compute all angle values in trajectory
                # This returns an [nframes x n_angles] array
                ang_val_array[file] = md.compute_angles(
                    traj, ang_sub_arrays[str(i + 1)]
                )
                ...
                # Reshape array:
                ang_val_array[file] = np.reshape(
                    ang_val_array[file], (nframes * (n_i[i] - 1)[0], 1)
                )
        ...
        torsion_bin_centers = np.zeros((len(torsion_bin_edges) - 1, 1))
        for i in range(len(torsion_bin_edges) - 1):
            torsion_bin_centers[i] = (
                torsion_bin_edges[i] + torsion_bin_edges[i + 1]
            ) / 2
        ...
        for i in range(i_torsion_type):
            if inv_torsion_dict[str(i + 1)] == backbone_torsion_type:
                ...
                # Reshape array
                torsion_val_array[file] = np.reshape(
                    torsion_val_array[file], (nframes * n_j[i][0], 1)
                )
...
def plot_2d_distribution(
    ...
    nrow = (
        int(np.ceil(np.sqrt(nseries)))
        + 2
        + (nseries % (np.ceil(np.sqrt(nseries))) != 0)
    )
    ...
    ax_south.tick_params(
        labelcolor="none", top=False, bottom=False, left=False, right=False
    )
    ax_north.tick_params(
        labelcolor="none", top=False, bottom=False, left=False, right=False
    )
...
def fit_ramachandran_data(hist_data, xedges, yedges):
    ...
    def two_gauss(xy, a, x0, y0, c):
        ...
        return a * np.exp(
            -(
                (np.power((x - x0), 2) + 0.5 * np.power((y - y0), 2))
                / (2 * np.power(c, 2))
            )
        )
    ...
    for key, value in hist_data.items():
        ...
        param_opt[key], param_cov[key] = curve_fit(
            two_gauss, xy_data, z_data.ravel(), param_guess
        )
