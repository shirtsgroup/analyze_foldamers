...
output_data = str(
    str(os.getcwd().split("examples")[0])
    + "examples/homopolymer_heat_capacity_varying_simulation_time/output/output5.0.nc"
)
...
configurations, energies = get_decorrelated_samples(
...
pitch_bins = np.array(
    [
        [min_pitch + i * pitch_step_size, min_pitch + (i + 1) * pitch_step_size]
    ...
    for pitch in data:
        for bin_index in range(len(pitch_bins)):
            ...
            if pitch > bin[0] and pitch <= bin[1]:
                pitch_bin_counts[data_index][bin_index] = (
                    pitch_bin_counts[data_index][bin_index] + 1
                )
    ...
    for radius in data:
        for bin_index in range(len(radius_bins)):
            ...
            if radius > bin[0] and radius <= bin[1]:
                radius_bin_counts[data_index][bin_index] = (
                    radius_bin_counts[data_index][bin_index] + 1
                )
    ...
    for residual in data:
        for bin_index in range(len(residual_bins)):
            ...
            if residual > bin[0] and residual < bin[1]:
                residual_bin_counts[data_index][bin_index] = (
                    residual_bin_counts[data_index][bin_index] + 1
                )

pitch_bins = np.array(
    [
        mean([min_pitch + i * pitch_step_size, min_pitch + (i + 1) * pitch_step_size])
        for i in range(n_bins)
    ]
)
radius_bins = np.array(
...
pyplot.legend(
    [round(temperature._value, 1) for temperature in temperature_list[:9]],
    loc="center left",
