    ...
    for index in range(len(bond_angles)):
        for bond_angle_range_index in range(len(bond_angle_ranges)):
            ...
            if bond_angle_range_index == 0:
                if float(bond_angles[index]) >= float(bond_angle_range[0]) and float(
                    bond_angles[index]
                ) <= float(bond_angle_range[1]):
                ...
                if float(bond_angles[index]) > float(bond_angle_range[0]) and float(
                    ...
                    bond_angle_bin_index = bond_angle_range_index
        bond_angle_bin_counts[bond_angle_bin_index] = (
            bond_angle_bin_counts[bond_angle_bin_index] + 1
        )
...
x = [
    ...
    for i in range(len(bond_angle_force_constant_list))
]
plot_distribution(
    x,
    bin_counts_list,
